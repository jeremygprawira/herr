...
// so a test can drive Log and decode the single emitted line.
func newCaptured(t *testing.T) (herr.Logger, *bytes.Buffer) {
	...
	return logrusadapter.New(l), buf
}

// decodeLine decodes the single JSON log line in buf.
...
// field becomes its own logrus field.
func TestLog_OptionalFields(t *testing.T) {
	...
	if m["status"] != float64(503) { // JSON numbers decode to float64
...
// TestNew_NilLoggerIsSafe proves a nil logger never panics.
func TestNew_NilLoggerIsSafe(t *testing.T) {
	...
	logrusadapter.New(nil).Log(context.Background(), herr.LogRecord(herr.New("X")))
}
